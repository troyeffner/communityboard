...
export async function GET() {
  ...
  const { data, error } = await supabase
    .from('poster_uploads')
    .select('id, file_path, status, type, created_at, processed_at, done, is_done, object_type, seen_at_category, seen_at_label, seen_at_name')
  ...
  if (error) {
    ...
    if (!fallbackWithSeenAtName.error) {
      uploads = ((fallbackWithSeenAtName.data || []) as Array<Omit<PosterUploadRow, 'processed_at' | 'type' | 'done' | 'is_done' | 'object_type' | 'seen_at_category' | 'seen_at_label'> & { seen_at_name?: string | null }>).map((row) => ({
        ...row,
        type: null,
        processed_at: null,
        done: false,
    ...
    uploads = ((fallback.data || []) as Omit<PosterUploadRow, 'processed_at' | 'type' | 'done' | 'is_done' | 'object_type' | 'seen_at_category' | 'seen_at_label' | 'seen_at_name'>[]).map((row) => ({
      ...row,
      type: null,
      processed_at: null,
      done: false,
  ...
  const uploadsWithCounts = uploads.map((u) => {
    ...
    return {
      ...
      seen_at_category: u.seen_at_category || null,
